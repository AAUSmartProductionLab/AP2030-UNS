...
namespace
{
    ...
    std::string classifyNodeCategory(const BT::TreeNode &node)
    {
        ...
        {
            if (reg_name.find("Selector") != std::string::npos || reg_name.find("Fallback") != std::string::npos)
            {
                return "selector";
            }
...
void BehaviorTreeController::loadAppConfiguration(int argc, char *argv[])
{
    ...
    std::cout << "  Metrics Topic Prefix: " << app_params_.metrics_topic_prefix << std::endl;
...
void BehaviorTreeController::setupMainMqttMessageHandler()
{
        ...
        [this](const std::string &topic, const nlohmann::json &payload, mqtt::properties props)
    {
        if (topic == this->app_params_.start_topic)
        {
            ...
            if (payload.contains("RunId") && payload["RunId"].is_string())
            {
                run_id = payload["RunId"].get<std::string>();
            }
            if (run_id.empty())
            {
            ...
            if (!this->sigint_received_.load())
            {
                ...
                }
                {
                    ...
                    this->current_run_id_ = run_id;
                }
                std::cout << "Received Start command RunId: " << run_id << std::endl;
...
void BehaviorTreeController::publishRunMetrics(BT::NodeStatus final_status)
{
    ...
    }
    {
        ...
        if (bt_observer_)
        {
            ...
            for (const auto &[uid, node_stats] : stats)
            {
                ...
                // ``operator[]`` on a missing key materializes a ``null`` JSON
                // value, and ``nlohmann::json::value(key, default)`` throws
                // ``type_error.306`` on non-object types. Initialize the
                // per-category bucket as an empty object before accumulating.
                ...
                bucket["transitions"] =
                    bucket.value("transitions", 0u) + node_stats.transitions_count;
                bucket["success"] =
                    bucket.value("success", 0u) + node_stats.success_count;
                bucket["failure"] =
                    bucket.value("failure", 0u) + node_stats.failure_count;
                bucket["skipped"] =
                    bucket.value("skipped", 0u) + node_stats.skip_count;
    ...
    if (mqtt_client_ && mqtt_client_->is_connected())
    {
        const std::string topic = app_params_.metrics_topic_prefix + "/bt_controller/" + sanitizeToken(run_id);
        if (!mqtt_client_->publish_message(topic, payload, 1, false))
        {
            std::cerr << "Failed to publish BT runtime metrics on topic " << topic << std::endl;
        }
        else
        {
            std::cout << "Published BT runtime metrics on topic " << topic << std::endl;
        }
    }
}
