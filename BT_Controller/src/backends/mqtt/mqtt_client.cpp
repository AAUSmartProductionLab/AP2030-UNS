...
PahoMqttClient::PahoMqttClient(std::string serverURI, std::string client_id,
...
mqtt::token_ptr MqttClient::subscribe_topic_paho(const std::string &topic, int qos)
{
    if (!is_connected())
    {
...
{
    auto token = subscribe_topic_paho(topic, qos);
...
{
    return mqtt::async_client::is_connected();
}

void PahoMqttClient::set_message_handler(MessageCallback handler)
{
...
bool MqttClient::publish_message_raw(const std::string &topic,
