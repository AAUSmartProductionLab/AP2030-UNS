...
NodeMessageDistributor::NodeMessageDistributor(IMqttClient &mqtt_client_ref)
                                                     ...
                                                     std::chrono::milliseconds timeout_per_subscription)
{
    ...
    std::map<std::string, std::vector<IMqttSubscriber *>> topic_to_instances_map;
    std::map<std::string, int> topic_to_max_qos;

    for (const auto &[type_idx, type_subscription_info] : node_subscriptions_)
    {
        for (IMqttSubscriber *instance : type_subscription_info.instances)
        {
            if (!instance)
                continue;
    ...
    for (const auto &handler : topic_handlers_)
    {
        try
        {
            auto token = mqtt_client_.subscribe_topic_paho(handler.topic, handler.qos);
            if (token)
            {
...
void NodeMessageDistributor::registerDerivedInstance(IMqttSubscriber *instance)
{
    if (!instance)
        return;
...
bool NodeMessageDistributor::registerLateInitializingNode(IMqttSubscriber *instance,
                                                          std::chrono::milliseconds timeout)
{
    ...
    for (const auto &[key, topic_obj] : topics)
    {
        ...
        bool handler_exists = false;
        {
            ...
            for (auto &h : topic_handlers_)
            {
                if (h.topic == topic_str)
                {
                    ...
                    for (IMqttSubscriber *existing : h.instances)
                    {
        ...
        try
        {
            ...
            auto token = mqtt_client_.subscribe_topic_paho(topic_str, qos);
            if (token)
            {
...
void NodeMessageDistributor::unregisterInstance(IMqttSubscriber *instance)
{
    if (!instance)
        return;
