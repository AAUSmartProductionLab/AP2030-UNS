...
def ontology_class_from_iri(value: str) -> str:
    """Return ontology class local-name for cssx/css IRIs or CURIEs.

    Supported forms:
    - http://www.w3id.org/aau-ra/cssx#Transport
    - http://www.w3id.org/hsu-aut/css#Resource
    - cssx:Transport
    - css:Resource

    Returns an empty string for non-ontology identifiers so callers can
    ...
    lowered = text.lower()
    if lowered.startswith("cssx:"):
        return text.split(":", 1)[1].strip()
    if lowered.startswith("css:"):
        return text.split(":", 1)[1].strip()

    if text.startswith("http://") or text.startswith("https://"):
        if "w3id.org/aau-ra/cssx#" not in lowered and "w3id.org/hsu-aut/css#" not in lowered:
            return ""
        return semantic_tail(text)
