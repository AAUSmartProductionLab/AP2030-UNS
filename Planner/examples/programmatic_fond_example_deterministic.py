...
def build_domain():
    ...
    product_at  = Predicate("product_at", p, s)   # product is at station (transport-agnostic)
    ...
    moveToStation = Action(
        ...
        effect=OneOf(
            ~at(c, s1) & at(c, s2) & ~product_at(p, s1) & product_at(p, s2),
            ~at(c, s1) & at(c, s2) & ~product_at(p, s1) & product_at(p, s2) & ~operational(c),
        ),
    )
    ...
    # Dispense product at a dispensing station (transport-agnostic)
    dispense = Action(
        "dispense", parameters=[p, s],
        precondition=(is_dispenser(s) & product_at(p, s)
    ...
    return domain, {
        ...
        "free_slot": free_slot, "product_at": product_at,
    }


def build_problem(domain, preds, n_products=2, capacities=None):
    ...
    *capacities*: dict mapping station kind → max concurrent products.
    ...
    """
    default_caps = {"shuttle": 2, "loader": 2, "dispenser": 1, "unloader": 1}
    caps = {**default_caps, **(capacities or {})}

    products      = constants(" ".join(f"p{i}" for i in range(n_products)),
                              type_="product")
    shuttle_objs  = constants(" ".join(f"shuttle{i}" for i in range(2)), type_="resource")
...
def main():
    ...
    problem = build_problem(domain, preds, n_products=2,
                            capacities={"shuttle": 1, "dispenser": 2})
