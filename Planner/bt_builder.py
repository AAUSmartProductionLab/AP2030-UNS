...
if TYPE_CHECKING:
    from pr2_adapter import FSAP, PR2Result, PolicyRule

from bt_causal import CausalInfo, build_causal_info
...
from bt_optimize import deduplicate_subtrees, parameterize_subtrees
...
def _strip_conditions(
    ...
    from pr2_adapter import PolicyRule
...
def _build_group_subtree(
    ...
    if group_common:
        from pr2_adapter import PolicyRule as _PR
        ...
        if len(plain_singles) >= 3:
            ...
            if family is not None:
                ...
                for lit in sorted(dispatch):
                    ...
                    from pr2_adapter import PolicyRule as PR
                    sub_rules = [
                        PR(
