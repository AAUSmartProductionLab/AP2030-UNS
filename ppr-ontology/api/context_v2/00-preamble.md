# ResourceAAS Generation Context — Preamble (v2 — IDTA-aligned)

You are generating an **Asset Administration Shell (AAS) JSON document** conforming to the AAS
Part 2 v3.1 specification and the **CSSx_AAS** ontology (which imports the official AAS v3.1
metamodel and adds domain-specific subclasses for our submodels).

## Mandatory semanticIds (IDTA-published)

Every submodel and every mandatory submodel element MUST include a `semanticId` whose key value
is **exactly one of the IRIs below**. The validator types each node by `semanticId`; missing or
non-matching IDs cause the node to fall through to bare `aas:Submodel` / `aas:Property` typing
and skip every domain constraint in CSSx_AAS.ttl.

**Submodel-level (set on each Submodel's `semanticId`):**

| Submodel idShort         | semanticId IRI |
|---|---|
| `DigitalNameplate`       | `https://admin-shell.io/zvei/nameplate/2/0/Nameplate` |
...
| `Capabilities`           | `https://admin-shell.io/idta/CapabilityDescription/1/0` |
| `Skills`                 | `https://smartproductionlab.aau.dk/CSSx/Skills/1/0/Submodel` |
| `OperationalData`        | `https://smartproductionlab.aau.dk/CSSx/OperationalData/1/0/Submodel` |
| `Parameters`             | `https://smartproductionlab.aau.dk/CSSx/Parameters/1/0/Submodel` |

**Submodel-element-level (set on each mandatory SME's `semanticId`):**
...
Use this exact structure — `type: ExternalReference` at the reference level and
