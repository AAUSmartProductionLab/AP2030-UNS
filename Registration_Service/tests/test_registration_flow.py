...
def test_config_parsing(config_path: str) -> dict:
    ...
    try:
        ...
        print_info(f"Broker: {mqtt.get('broker_host')}:{mqtt.get('broker_port')}")
        
        # Test actions
        actions = parser.get_actions()
        print_success(f"Actions found: {len(actions)}")
        for action in actions:
            name = list(action.keys())[0] if isinstance(action, dict) else action.get('name', 'unknown')
            print_info(f"- {name}")
        ...
        for prop in properties:
            name = list(prop.keys())[0] if isinstance(prop, dict) else prop.get('name', 'unknown')
            print_info(f"- {name}")
...
def test_aas_generation(config_path: str, output_dir: str) -> dict:
    ...
    try:
        ...
        aas_data = generator.generate_system(system_id, system_config, return_store=False)
        ...
        for sm in submodels:
            ...
            print_info(f"Submodel: {sm.get('idShort')} - {len(sm_elements)} elements")
...
def main():
    ...
    parser.add_argument('--basyx-url', default='http://192.168.0.104:8081',
                        help='BaSyx server URL')
    ...
    try:
        ...
        results['topics_generation'] = test_topics_generation(str(config_path), output_dir)
        results['databridge_generation'] = test_databridge_generation(str(config_path), output_dir)
        results['aas_generation'] = test_aas_generation(str(config_path), output_dir)
        ...
        for test_name, result in results.items():
            ...
            if status == 'PASSED':
                ...
                print(f"{Colors.YELLOW}⚠ {test_name}: PASSED WITH WARNINGS{Colors.END}")
                passed += 1
            elif status == 'SKIPPED':
